...
/**
 * La clé du titre d'onglet pour un chemin.
 *
 * Les métadonnées de page — `layout.tsx` — sont écrites en français, et le
 * resteront : elles sont produites sur le serveur, servent aux moteurs de
 * recherche et aux aperçus de lien, et les faire dépendre de la langue du
 * visiteur obligerait à rendre dynamiquement quarante-cinq routes qui sont
 * aujourd'hui statiques. Ce qui se voit, en revanche, c'est le nom de l'onglet,
 * et lui n'a aucune raison de rester en français quand le reste de l'écran ne
 * l'est pas.
 *
 * On ne tient pas une seconde table pour autant : la navigation en a déjà une,
 * qui dit `/finales` → `nav.endgames`. On cherche donc ici le chemin connu le
 * plus long qui préfixe celui qu'on affiche — `/apprendre/principes` avant
 * `/apprendre` —, et l'on rend sa clé.
 */
export function cleDuTitre(pathname: string): TranslationKey | null {
  const chemin = pathname.split(/[?#]/)[0] ?? pathname

  const candidats: Array<{ href: string; labelKey: TranslationKey }> = [
    ...SECTIONS.flatMap((section) => [
      ...(section.sommaire ? [{ href: section.sommaire, labelKey: section.labelKey }] : []),
      ...section.entrees,
    ]),
    ...PAGES_APPLICATION,
    ...RACCOURCIS_MOBILES,
  ...
  let meilleur: { href: string; labelKey: TranslationKey } | null = null
  for (const entree of candidats) {
    const propre = entree.href.split(/[?#]/)[0] ?? entree.href
    // `'/'` préfixe tout : il ne vaut que pour lui-même, comme dans `estActif`.
    const colle = propre === '/' ? chemin === '/' : chemin.startsWith(propre)
    if (!colle) continue
    if (!meilleur || propre.length > meilleur.href.length) {
      meilleur = { href: propre, labelKey: entree.labelKey }
    }
  }

  return meilleur?.labelKey ?? null
}
